...
Private Sub PrintOrderProcessing()
    ...
    End If
        ...
        SQL = "SELECT DISTINCT (Select PrintName From AccountMaster Where Code=P.Party) AS AccountName,IIF(LEFT(BOM,4)+Right(BOM,2)IN ('0110FI','0510FI','0101FI','0501FI','0310FI','0710FI','0301FI','0701FI'),'GRN','GDN')+'/'+LTRIM(P.Name)+'-FG/19-21' AS VchNo,P.Date AS VchDate,(Select PrintName From BookMaster Where Code=C.Item ) AS ItemName,P.ChallanNo,(P.ChallanDate),IIF(LEFT(BOM,4)+Right(BOM,2)IN ('0110FI','0510FI','0101FI','0501FI','0310FI','0710FI','0301FI','0701FI','0210FI','0610FI','0201FI','0601FI','0410FI','0810FI','0401FI','0801FI'),ABS(C.Quantity),0) AS Quantity,IIF(LEFT(BOM,4)+Right(BOM,2)IN ('0110FI','0510FI','0101FI','0501FI','0310FI','0710FI','0301FI','0701FI'),ABS(C.Quantity),0) AS Received," & _
                  "IIF(LEFT(BOM,4)+Right(BOM,2)IN ('0210FI','0610FI','0201FI','0601FI','0410FI','0810FI','0401FI','0801FI'),ABS(C.Quantity),0) AS Issued,(P.Party) AS BCode,LTRIM(P.Name) AS GRNNo,P.Date,(Select Name From AccountMaster Where Code=P.Consignee) AS Consignee,(Select Name From AccountMaster Where Code=P.Party) AS Party,P.Remarks,LTRIM(P.Name) AS PO,(Select PrintName From BookMaster Where Code=C.Item ) AS Book,C.Quantity As Qty,C.Rate,C.Amount,P.BOX,P.Freight,P.TYPE, Right(P.Type,2)+'-'+LTRIM(P.Name) As MRNNo,Right(P.Type,2) As OrderType,C.Item,IIF(LEFT(P.Type,4)='0110','Purchase',IIF(LEFT(P.Type,4)='0210','Purchase Return',IIF(LEFT(P.Type,4)='0510','Pur Challan IN',IIF(LEFT(P.Type,4)='0610','Pur Challan Out',IIF(LEFT(P.Type,4)='0310','Sales Return',IIF(LEFT(P.Type,4)='0410','Sales',IIF(LEFT(P.Type,4)='0710','Sales Challan IN',IIF(LEFT(P.Type,4)='0810','Sales Challan Out','Order Processing')))))))) As TypeRef,C.BOM  As BOM,P.Code,P.Remarks AS RemarkC " & _
                  "FROM JobworkBVParent P INNER JOIN JobworkBVChild C ON P.Code=C.Code Left Join JobworkBVRef R ON R.VchCode=C.Code " & _
                  "WHERE LEFT(BOM,4)+Right(BOM,2)IN ('0110FI','0510FI','0101FI','0501FI','0310FI','0710FI','0301FI','0701FI','0210FI','0610FI','0201FI','0601FI','0410FI','0810FI','0401FI','0801FI') AND LEFT(P.Code,1)<>'*' AND P.Date>=#" & GetDate(MhDateInput1.Text) & "# AND P.Date<=#" & GetDate(MhDateInput2.Text) & "# AND " & _
                  IIf(Option3.Value, "LEFT(BOM,4)+Right(BOM,2)IN ('0210FI','0610FI','0201FI','0601FI','0410FI','0810FI','0401FI','0801FI')", IIf(Option1.Value, "LEFT(BOM,4)+Right(BOM,2)IN ('0110FI','0510FI','0101FI','0501FI','0310FI','0710FI','0301FI','0701FI')", IIf(Option2.Value, "LEFT(BOM,4)+Right(BOM,2)IN ('0110FI','0510FI','0101FI','0501FI','0310FI','0710FI','0301FI','0701FI','0210FI','0610FI','0201FI','0601FI','0410FI','0810FI','0401FI','0801FI')", 1))) & "  " & _
                  "AND (P.Party) IN (" & SelectedItems(ListView1) & ") AND C.Item IN (" & SelectedItems(ListView2) & ") " & _
